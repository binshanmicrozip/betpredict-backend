    ...
    def handle(self, *args, **options):
        ...
        for d in deliveries:
            if d.batter_id:
                ...
                if d.player_out_id == d.batter_id and (d.wicket_kind or "").lower() not in WICKET_EXCLUDE_FOR_BATTER_OUT:
        ...
        PlayerSituationStats.objects.bulk_create(objs, batch_size=1000)

        self.stdout.write("Updating last season...")

        season_map = {
            row["player_id"]: row["last_season"]
            for row in MatchPlayer.objects.values("player_id").annotate(
