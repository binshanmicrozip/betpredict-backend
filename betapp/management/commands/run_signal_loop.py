        ...
        while True:
            try:
                ...
                payload = {
                    ...
                    "publish_time_ms": int(timezone.now().timestamp() * 1000),
                    "publish_time_utc": timezone.now(),
                    "ltp": ltp,
                    "prev_ltp": prev_ltp,
                    "market_status": "OPEN",
                    "in_play": True,
                ...
                self.stdout.write(self.style.SUCCESS(
                    f"[LiveMarketTick] saved market_id={market_id} runner_id={runner_id} "
                    f"runner_name={metadata.get('runner_name')} event_name={metadata.get('event_name')} ltp={ltp}"
                ))
